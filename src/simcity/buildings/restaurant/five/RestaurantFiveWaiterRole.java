...
public class RestaurantFiveWaiterRole extends Role implements RestaurantFiveWaiter {
	...
	private void SendOrderToCook(MyCustomer c) {
		...
		AlertLog.getInstance().logMessage(AlertTag.valueOf(restaurant.getName()), "RestaurantFiveWaiter: " + person.getName(), "Sending " + c.c.getName() + "'s order of " + c.choice + " from table " + c.table + " to cook.");
	...
	private void ServeFood(Order o) {
		...
		try {
			for(MyCustomer mc : customers) {
				if(mc.table == o.table) { //mc.table replaced mc.c.tableNum
					...
					((RestaurantFiveWaiterGui) gui).DoGoToTable(mc.table);
					try {
						atDest.acquire();
					} catch (InterruptedException e) {
					}
					AlertLog.getInstance().logMessage(AlertTag.valueOf(restaurant.getName()), "RestaurantFiveWaiter: " + person.getName(), "Serving " + mc.choice + " to " + mc.c.getName() + ".");

					mc.s = CustomerState.eating;
					mc.c.msgHereIsYourFood(mc.choice);

					//asking cashier to produce check for this customer
					AlertLog.getInstance().logMessage(AlertTag.valueOf(restaurant.getName()), "RestaurantFiveWaiter: " + person.getName(), "Asking cashier for check.");
					restaurant.getCashier().msgProduceCheck(this, mc.c, mc.choice);
					return;
				}
			}
		}
